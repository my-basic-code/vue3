<template>
  <article>
    <Input label="연락처" type="number" name="phone" disabled placeholder="연락처를 입력해주세요" wrapClass="mt-7"
    ...
    <Button :class="`w-full py-4 px-9 mt-4 ${isSendCodeAgain ? classBtn[2] : classBtn[3]
      }`" :disabled="!isSendCodeAgain" @click="sendCodeAgain">인증번호 받기
    </Button>
    <div class="flex items-end space-x-[10px]">
      <Input label="인증번호 입력" name="verifyCode" type="number" :maxLength="6" wrapClass="mt-7 relative"
        classLabel="block text-base font-bold" :className="` px-5 py-4 mt-[6px] w-full ${classInputCustom[2].input}`"
        :disabled="isSendCodeAgain" v-model="isVerifyCode">
      <template #sub-label>
        <span class="text-xs text-[#FF3609] font-normal">*</span>
      </template>
      <template #suffix>
