    ...
    <ul class="flex items-center justify-center gap-x-3">
      <li v-if="currentPage > 1" class="cursor-pointer">
        <a @click="changePage(currentPage - 1)">
          <svg style="transform: rotate(180deg);" xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="none"
            viewBox="0 0 24 24">
      ...
      <li v-for="pageNumber in visiblePages" :key="pageNumber">
        <a class="w-[32px] h-[24px] text-[12px] font-bold flex justify-center items-center cursor-pointer text-[#8B8B8B]"
          :class="{ 'text-black': currentPage === pageNumber }" @click="changePage(pageNumber)">{{ pageNumber }}</a>
      </li>
